    ...
    @foreach ($activities as $activity)
        <div class="activity-container">
            ...
            <div class="activity-user-info">
                <img class="rounded-circle shadow-1-strong activity-user-photo" src="{{ asset($activity->user->user_photo) }}" alt="User" />
                <div class="">
                    ...
                    @if($clasification)
                        <div class="activity-score-info">
                            <div class="activity-game-note-info">
                                <p class="activity-game-number comentarios">Game number : {{ $activity->game->number }}</p>
                            ...
                            <div class="activity-player-champion-info">
                                <img class="rounded-circle shadow-1-strong activity-player-photo" src="{{ asset($activity->player->photo) }}" alt="{{ $activity->player->nick }}" />
                                <img class="rounded-circle shadow-1-strong activity-champion-photo" src="{{ asset($clasification->champion->square) }}" alt="{{ $clasification->champion->name }}" />
                                <p class="comentarios">KDA: {{ $clasification->kills }} / {{ $clasification->deaths }} / {{ $clasification->assists }}</p>
                            </div>
                        </div>
                    @else
