        ...
        <div class="row">
            <div class="col-md-12">
                <div class="custom-div-2">
                    <img src="{{ asset($serie->competition->logo) }}" alt="{{ $serie->competition->name }}"
                        class="w-auto h-40 logo">
                    <h2>{{ $serie->competition->name }}</h2>
                    <h2>{{ $serie->name }}</h2>
                    <h2>{{ $serie->date }}</h2>
                    <h2>{{ $serie->type }}</h2>
                    <a href="#" data-bs-toggle="modal" data-bs-target="#editSerieModal"
                        class="btn btn-primary d-block mb-2 edit-button">Edit serie</a>
                </div>
                                ...
                                @foreach ($serie->games as $game)
                                    <tr class="row-color text-center">
                                        <td>
                                            ...
                                            <img src="{{ asset($game->team_blue->logo) }}"
                                                alt="{{ $game->team_blue->name }}" class="w-36 h-auto mx-auto d-block">
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            <img src="{{ asset($game->team_red->logo) }}"
                                                alt="{{ $game->team_red->name }}" class="w-36 h-auto mx-auto d-block">
                                        </td>
                                        <td>
                                            <p>{{ $game->number }}</p>
                                        </td>
