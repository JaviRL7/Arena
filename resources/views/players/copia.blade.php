        ...
        <div class="col-md-6">
            ...
            <div class="player-profile-stadistics" style="display: flex; align-items: center;">
                @if ($player->mostPlayedChampion())
                    <h1 style="font-family: important; color:white; margin-right: 280px;">
                        {{ $player->mostPlayedChampion()->name }}</h1>
                    <img src="{{ asset($player->mostPlayedChampion()->square) }}" alt=""
                        class="rounded-circle">
                @else
                    <h5 style="font-family: important; color:white; margin-right: 280px;">This player has not played any
                        matches yet.</h5>
                @endif
            </div>
                ...
                <div class="col-md-12">
                    ...
                    @if (!$isFan)
                        <form action="{{ route('players.addFan', ['player' => $player->id]) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-primary" data-player-id="{{ $player->id }}">Become
                                a Fan</button>
                        </form>
                    @else
                        <form action="{{ route('players.removeFan', ['player' => $player->id]) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn btn-primary" data-player-id="{{ $player->id }}">Dejar
                                de ser fan</button>
                        </form>
                    @endif
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            alert(
                                'Hubo un error al intentar dejar de ser fan. Por favor, inténtalo de nuevo.');
                        }
                    }
                });
            } else {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.maxFavoritesReached) {
                            ...
                            // Muestra un mensaje de error
                            alert(
                                'Hubo un error al intentar añadir a este jugador a tus favoritos. Por favor, inténtalo de nuevo.');
                        }
                    }
                });
            }
